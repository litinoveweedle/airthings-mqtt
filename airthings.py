...
class WavePlus:
    def __init__(self, SerialNumber, timeout=10):
    ...
    def _run_with_timeout(self, description, func):
        if not hasattr(signal, "SIGALRM"):
            return func()

        def timeout_handler(signum, frame):
            raise TimeoutError(f"{description} exceeded {self.timeout}s")
    ...
    def connect(self):
        ...
        if self.periph is None:
            self.periph = self._run_with_timeout(
                "Bluetooth connect",
                lambda: Peripheral(self.MacAddr),
            )
        if self.curr_val_char is None:
            characteristics = self._run_with_timeout(
                "Bluetooth characteristic lookup",
                lambda: self.periph.getCharacteristics(uuid=self.uuid),
            )
            self.curr_val_char = characteristics[0]
        return True
    ...
    def read(self):
        if self.curr_val_char is None:
            ...
            return False
        rawdata = self._run_with_timeout(
            "Bluetooth read",
            lambda: self.curr_val_char.read(),
        )
...
class Sensors:
    def __init__(self):
        ...
        self.sensors = {
            ...
            "RADON_LONG_TERM_AVG": "Bq/m3",
            "TEMPERATURE": "degC",
            "REL_ATM_PRESSURE": "hPa",
            "CO2_LVL": "ppm",
            "VOC_LVL": "ppb",
        }
        self.sensor_data = {}
